...
mean_FDR <- function(n, N, p, type, params = NULL, FDR) {
...
results_list <- list()
for (i in 1:length(l)){
  k<-mean_FDR(200,100,400,type="AR1",params=list(rho=0.8),FDR=l[i])
  results_list[[as.character(i)]] <- k
}
results_df <- do.call(rbind, results_list)
write.csv(results_df,file="FDR_AR1.csv", row.names = TRUE) 
results_df
read.csv("FDR_AR1.csv")

l=c(0.01,0.05,0.10,0.15,0.2)
results_list <- list()
for (i in 1:length(l)) {
  k<-mean_FDR(200,100,400,type="BlockDiagonal",params=list(rho=0.8),FDR=l[i])
  results_list[[as.character(i)]] <- k
}
results_df <- do.call(rbind, results_list)
write.csv(results_df,file="FDR_BlockDiagonal.csv", row.names = TRUE)

l=c(0.01,0.05,0.10,0.15,0.2)
results_list <- list()
for (i in 1:length(l)) {
  k<-mean_FDR(200,100,400,type="SmallWorld",params=list(rho=0.8),FDR=l[i])
  results_list[[as.character(i)]] <- k
}
results_df <- do.call(rbind, results_list)
write.csv(results_df,file="FDR_SmallWorld.csv", row.names = TRUE)

l=c(0.01,0.05,0.10,0.15,0.2)
results_list <- list()
for (i in 1:length(l)) {
  k<-mean_FDR(200,100,400,type="StarGraph",params=list(rho=0.8),FDR=l[i])
  results_list[[as.character(i)]] <- k
}
results_df <- do.call(rbind, results_list)
write.csv(results_df,file="FDR_StarGraph.csv", row.names = TRUE)
