...
mean_FDR_TPR <- function(n, N, p, type, params = NULL) {
  ...
  for (i in 1:N) {
    ...
    W <- stat.lasso_coefdiff(X,X_tilde,y.sample)
...
results_df
write.csv(results_df,file="FDR_AR1_upk3.csv", row.names = TRUE)
read.csv("FDR_AR1_upk3.csv")

results_df<-mean_FDR_TPR(200,100,400,type="BlockDiagonal",params=list(rho=0.8))
results_df
write.csv(results_df,file="FDR_BlockDiagonal_upk3.csv", row.names = TRUE)

results_df<-mean_FDR_TPR(200,100,400,type="SmallWorld",params=list(rhos=c(0.8,0.7,0.6)))
results_df
write.csv(results_df,file="FDR_SmallWorld_upk3.csv", row.names = TRUE)

results_df<-mean_FDR_TPR(200,100,400,type="StarGraph",params=list(rho=0.8))
results_df
write.csv(results_df,file="FDR_StarGraph_upk3.csv", row.names = TRUE)